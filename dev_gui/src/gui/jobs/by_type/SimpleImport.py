...
from appli.gui.jobs.Job import Job, load_from_json
...
class SimpleImportJob(Job):
    ...
    def initial_dialog(cls):
        ...
        with ApiClient(ProjectsApi, request) as api:
            try:
                _target_prj: ProjectModel = api.project_query(
    ...
    def create_or_update(cls):
        ...
        with ApiClient(ProjectsApi, request) as api:
            rsp: SimpleImportRsp = api.simple_import(
                project_id=prj_id, simple_import_req=req, dry_run=True
            )
        ...
        if not cls.flash_any_error(errors):
            ...
            with ApiClient(ProjectsApi, request) as papi:
                rsp2: SimpleImportRsp = papi.simple_import(
                    project_id=prj_id, simple_import_req=req, dry_run=False
                )
                job_id = rsp2.job_id
        ...
        return PrintInCharte(render_template("jobs/simpleimport.html", preset=values))
    ...
    def final_action(cls, job: JobModel):
        ...
        return (
            "Imported %s images successfully<br><a href='/prj/%s' class='btn btn-primary'>Go to project</a>"
            % (obj_count, prj_id)
        )
