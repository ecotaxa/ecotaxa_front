...
class UsersView(APIModelView):
    ...
    # Additions
    mandatory_columns = ("email", "name", "country", "usercreationdate")
    ...
    def update_model(self, form, model):
        ...
        form_fields["usercreationdate"].data = (
            form_fields["usercreationdate"].data.isoformat() + " 00:00:00"
        )
        try:
            with ApiClient(UsersApi, request) as api:
                ret = api.update_user(
                    user_id=form.data["id"], user_model_with_rights=form.data
                )
    ...
    def edit_form(self, obj=None):
        ...
        form.can_do.choices = [
            (str(num), lbl) for num, lbl in constants.API_GLOBAL_ROLES.items()
        ]
        return form
    ...
    def scaffold_form(self):
        ...
        form_class.password_confirm = PasswordField(
            "Password Confirmation", validators=[self.check_password_confirm]
        )
        return form_class
