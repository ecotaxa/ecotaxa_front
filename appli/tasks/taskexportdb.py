...
from appli import db, app, database, PrintInCharte, gvp
...
class TaskExportDb(AsyncTask):
    ...
    def SPStep1(self):
        ...
        cmd += "  --schema-only --format=p  -f schema.sql -E LATIN1 -n public --no-owner  --no-privileges " \
               "--no-security-labels --no-tablespaces  " + \
               app.config['DB_DATABASE'] + "  >dumpschemaout.txt 2>&1"
        ...
        self.pgcur.execute(
            "select imgid,file_name,thumb_file_name "
            "  from images i "
            "  join obj_head o on o.objid=i.objid "
            " where o.projid in (%s)" % (self.param.ProjectIds,))
        for r in self.pgcur:
            if r[1]:
    ...
    def QuestionProcess(self):
        ...
        if self.task.taskstep == 0:
            ...
            if gvp('starttask') == "Y":
                ...
                if self.param.ProjectIds == '':
                    errors.append("You must select at least one project")
                if len(errors) > 0:
                    for e in errors:
                        flash(e, "error")
            ...
            SELECT PROJECT(S) TO INCLUDE IN EXPORTED DATABASE <a name="tbltop" href="#tbltop" 
            onclick="$('#TblPrj input').prop( 'checked', true )">All</a> /
...
<br>2.	Move the project into the mobile application as described above 
(no need to delete the project because it is empty)
<br>3.	After the cruise, move (see above) the (full) project from the mobile application to the server application 
and delete it from the mobile application
