from flask_babel import _

py_messages = dict(
    {
        "filetoloaderror": _("At least one object does not belong to you"),
        "cannotpurgeprj": _("You cannot Purge this project"),
        "cannotmergeprj": _("You cannot merge from this project"),
        "sourceprjnotfound": _("Source project doesn't exist"),
        "objsnotincurrentprj": _("object(s) are not in current project."),
        "prjdestroyed": _("Project and associated privileges, destroyed"),
    }
)
