...
def indexPrj(PrjId):
    ...
    arr_url = {
        ...
        "prediction": "/Job/Create/Prediction?projid=%d" % PrjId,
        "predictionf": "javascript:GotoWithFilter('/Job/Create/Prediction')",
        "import": "/gui/job/create/FileImport?projid=%d" % PrjId,
        "taxofix": "/prj/taxo_fix/%d" % PrjId,
...
def LoadRightPaneForProj(PrjId: int, read_only: bool, force_first_page: bool):
    ...
    for obj_dtl in objs.details:
        ...
        name_chunk = FormatNameForVignetteDisplay(
            display_name, taxostatus, hyphenator, categ_cache
        )
        if g.PublicViewMode:
            simsrch_btn = ""
        else:
...
def prjGetClassifTab(PrjId):
    ...
    for a_taxon in taxa:
        ...
        for taxon_id, taxon_name, taxon_status in zip(
            reversed(a_taxon.id_lineage),
            reversed(a_taxon.lineage),
            reversed(a_taxon.lineage_status),
        ):
    ...
    return render_template(
        "project/classiftab.html",
        res=restree,
        taxotree=json.dumps(taxotree),
        projid=PrjId,
    )
