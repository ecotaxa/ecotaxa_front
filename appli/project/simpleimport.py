...
from appli import app, PrintInCharte, database, gvp, CreateDirConcurrentlyIfNeeded, UtfDiag3
...
def CreateObject(Prj, Metadata, ImageName, ImageFile):
    ...
    vaultfilenameThumb = "%s/%04d_mini%s" % (
        VaultFolder, Img.imgid % 10000, '.jpg')  # on Impose le format de la miniature
...
def prj_simpleimport(PrjId):
    ...
    if gvp("doimport", 'N') == 'N':
        ...
        # preset = {"imgdate": "20150327", "imgtime": "1447",
        # "latitude": "-12.06398", "longitude": "-135.05325","depthmin": "50", "depthmax": "70","userlb":8,"status":'V'}
        ...
        # instrum=GetAll("select * from (SELECT DISTINCT  instrument  from
        # acquisitions where instrument is not null) q order by upper(instrument)")
        # ,instrum=instrum
    ...
    if gvp("doimport", 'N') == 'Y':
        ...
        fieldlist = ("imgdate", "imgtime", "latitude", "longitude",
        ...
        if Metadata["imgdate"]:
            if len(Metadata["imgdate"]) != 8:
                return PrintError(
                    "Invalid Date size '%s' for Field Image date . Must be 8 digit YYYYMMDD" % (Metadata["imgdate"],),
                    DefBodyMsg)
            try:
                datetime.date(int(Metadata["imgdate"][0:4]), int(Metadata["imgdate"][4:6]),
                              int(Metadata["imgdate"][6:8]))
            except ValueError:
                return PrintError("Invalid Date value '%s' for Field Image date ." % (Metadata["imgdate"],), DefBodyMsg)
        if Metadata["imgtime"]:
            if len(Metadata["imgtime"]) != 4:
                return PrintError(
                    "Invalid Time size '%s' for Field Image Time . Must be 4 digit HHMM" % (Metadata["imgtime"],),
                    DefBodyMsg)
            try:
        ...
        txt += "Imported %s images successfully<br><a href='/prj/%s' class='btn btn-primary'>Go to project</a>" % (
            ObjCount, Prj.projid)
