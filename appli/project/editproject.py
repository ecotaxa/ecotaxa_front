...
def PrjEdit(PrjId, privs_only=False):
    ...
    if gvp("save") == "Y":
        # list all users in list_users key:'id'
        users_list = {}
    ...
    with ApiClient(UsersApi, request) as api:
        all_users: List[MinUserModel] = api.search_user(by_name="%%")
    for a_user in sorted(all_users, key=lambda u: u.name.strip().lower()):
        users[a_user.id] = a_user.name.strip()
    members = []
    for a_priv, l_members in members_by_right.items():
        for a_user in l_members:
            members.append(
                {
                    "member_id": a_user.id,
                    "name": a_user.name.strip(),
                    "privilege": a_priv,
                }
            )
    ...
    scn = {a_model.name: {"name": a_model.name} for a_model in possible_models}
