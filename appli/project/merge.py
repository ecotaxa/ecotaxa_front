...
def PrjMerge(PrjId):
    ...
    if not gvg("src"):
        ...
        txt += (
            """<ul><li>You are allowed to merge projects that you manage and contain images from the <b>same</b> instrument.
...
<li>Acquisitions with same acq_id on both sides will <b>not</b> be updated from added project.
        ...
        with ApiClient(ProjectsApi, request.cookies.get("session")) as api:
            rsp: List[ProjectModel] = api.search_projects(
                for_managing=True, instrument_filter=target_proj.instrument
            )
        ...
        for r in rsp:
            ...
            <td>{pctclassified:0.2f}</td>
                ...
                activeproject=target_proj.projid, **r.to_dict()
            )
        txt += "</table>"
    ...
    txt += """<h4>Source Project : {0} - {1} (This project will be destroyed)</h4>
            """.format(
        source_proj.projid, XSSEscape(source_proj.title)
    )
    ...
    if not gvg("merge"):  # Ici la src à été choisie et vérifiée
        ...
        if len(rsp2.errors) == 0:
            ...
            """,
                target_proj.projid,
                source_proj.projid,
                XSSEscape(source_proj.title),
                DoNotEscape=True,
            )
            return PrintInCharte(txt)
        else:
            return PrintInCharte(
                'Hit "Back" on the navigator to pick another source project.'
            )
    ...
    if gvg("merge") == "Y":
        ...
        txt += (
            "<br><a class='btn btn-lg btn-primary' href='/prj/%s'>Back to target project</a>"
            % target_proj.projid
        )
        return PrintInCharte(txt)
