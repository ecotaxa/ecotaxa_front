    ...
    if gvp("objlist") == "":
        ...
        if len(filtres):
            ...
            txt += (
                ...
                "USING Active Project Filters, {0} objects</span>".format(
                    len(object_ids)
                )
            )
        else:
        ...
        if gvp("objlist") == "DELETEALL":
            ...
            with ApiClient(ProjectsApi, request) as api:
                no, noh, ni, nbrfile = api.erase_project(
                    project_id=PrjId, only_objects=gvp("destroyproject") != "Y"
                )
        else:
            ...
            if err is not None:
                ...
                return PrintInCharte(
                    txt
                    + "<br><br><a href ='/prj/{0}'>Back to project home</a>".format(
                        PrjId
                    )
                )
        ...
        txt += (
            "Deleted %d Objects, %d ObjectHisto, %d Images in Database and %d files"
            % (no, noh, ni, nbrfile)
        )
        ...
        if gvp("objlist") == "DELETEALL" and gvp("destroyproject") == "Y":
            ...
            return PrintInCharte(
                txt + "<br><br><a href ='/prj/'>Back to project list</a>"
            )

    return PrintInCharte(
        txt + "<br><br><a href ='/prj/{0}'>Back to project home</a>".format(PrjId)
    )
