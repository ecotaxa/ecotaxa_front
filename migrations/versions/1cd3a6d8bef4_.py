...
def upgrade():
    ...
    op.add_column('users', sa.Column('usercreationdate', postgresql.TIMESTAMP(), nullable=True))
    op.add_column('users', sa.Column('usercreationreason', sa.String(length=1000), nullable=True))
    # ### end Alembic commands ###
    op.execute("""with r as (select min(id) id from roles where name like '%roject creator')
...
""")
    op.execute("update roles set name='Project creator' where name='Particle module project creator'")
    op.execute("""INSERT INTO countrylist(countryname) VALUES
('Afghanistan'),
('Albania'),
('Algeria'),
('America'),
